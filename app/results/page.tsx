...
import { TResultMetric } from "@/lib/types";
...
function Page() {
  ...
  if (!info || !isMounted || !gender) return null;
...
const Summary = ({ gender }: { gender: string }) => {
  return (
    <section className="flex flex-col items-center">
      <div className="flex items-center mt-6">
        ...
        <div className="h-[43px] border border-compliment ml-[50.5px] mr-5" />
...
const ResultMetric = ({ metric }: { metric: TResultMetric }) => {
  return (
    <div
      key={metric.id}
      className="flex items-center w-full h-[72px] border border-compliment rounded-lg px-3 gap-x-3"
    >
